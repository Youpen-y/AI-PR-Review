...
### Problem: Creating goroutines for each task causes exhaustion
...
func ProcessItems(items []Item) []Result {
    ...
    for i, item := range items {
        // Create goroutine for each item
        go func(idx int, it Item) {
            results[idx] = ProcessItem(it)  // Data race!
        }(i, item)
    }

    // Don't know when goroutines finish
    return results
}

// Problems:
// 1. Unlimited goroutines → resource exhaustion
// 2. Data race on results slice
...
// Benefits:
// 1. Limited goroutines (worker pool pattern)
// 2. No data races (each goroutine owns its index)
// 3. Proper synchronization with WaitGroup
```
